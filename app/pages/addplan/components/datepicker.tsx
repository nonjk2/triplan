...
class Calendars extends Component {
  ...
  // 날짜와 날짜 사이값 채워넣기 //
  getDatesStartToLast(lastday: any, firstday: any) {
    var regex = RegExp(/^\d{4}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])$/);
    if (
      !(
        regex.test(this.state.selectday[0]) &&
        regex.test(this.state.selectday[1])
      )
    ) {
    ...
    const date2 = result.slice(1, result.length - 1).map(e =>
      Object.assign({
        [e]: {
          selected: true,
          color: '#5585E8',
  ...
  daypicked = day => {
    if (this.state.selectday.length === 0) {
      this.setState(
        ...
        () => {
          let obj = this.state.selectday.reduce(
            (c, v) =>
              Object.assign(c, {
                [v]: {
                  // startingDay: true,{}
      ...
      if (this.state.selectday[0] === day) {
        ...
        this.setState(
          ...
          () => {
            ...
            let obj2 = this.state.selectday.reduce(e =>
              Object.assign({
                [e]: {
                  marked: true,
                  startingDay: true,
