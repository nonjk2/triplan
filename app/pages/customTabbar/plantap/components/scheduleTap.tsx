...
function ScheduleTap(props: any) {
  const {plan_id} = props;
  const [planday] = useState(props.planday);
  ...
  const [routes] = useState(plandays());
  ...
  const _handleIndexChange = (index: any) => setTabindex(index);
  ...
  return planday !== 1 ? (
    ...
    <ScheduleList
      navigation={props.navigation}
      startDate={new Date(props.startDate).getDate()}
    />
  );
